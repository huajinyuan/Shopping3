...
//            .merchantUserAgreementUri(Uri.parse("https://www.example.com/legal"));
    ...
    @OnClick({R.id.img_topbar_back, R.id.tv_action_pay_1, R.id.tv_action_pay_2})
    public void OnClick(View v) {
        switch (v.getId()) {
            case R.id.tv_action_pay_1:
                onBuyPressed();
                break;
            case R.id.tv_action_pay_2:
                onBuyPressed();
                break;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_PAYMENT) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (confirm != null) {
                    try {
                        F.e(confirm.toJSONObject().toString(4));
                        F.e(confirm.getPayment().toJSONObject().toString(4));
                        /**
